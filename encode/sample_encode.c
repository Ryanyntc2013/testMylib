...
//    char c;
    ...
    if( NULL != vEncStream ) {
        ...
        for( i = 0; i < vEncStream->u32PackCount; ++i ) {
            ...
            len = vEncStream->pstPack[i].u32Len;
            ...
            if (unal_type == H264E_NALU_SPS) {
                set_h264SeqParam(channel, 0, &pstr[4], proDataInfo.nalInfo.nalSize[i] - 4, 0);
            } else if (unal_type == H264E_NALU_PPS) {
                set_h264PicParam(channel, 0, &pstr[4], proDataInfo.nalInfo.nalSize[i] - 4, 0);
            }
            ...
            if ( len > 0 ) {
                proDataInfo.count         = index + 1;
                proDataInfo.buf[index]     = (char *)vEncStream->pstPack[i].pu8Addr;
