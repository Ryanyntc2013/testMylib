...
static PAYLOAD_TYPE_E s_enPayloadType = PT_G711A;//PT_ADPCMA;// 
...
HI_S32 SAMPLE_StartAi(AUDIO_DEV AiDevId, HI_S32 s32AiChnCnt,AIO_ATTR_S *pstAioAttr)
{
    ...
    for (j=0; j<s32AiChnCnt; j++)
    {
...
#if 0
        s32Ret = HI_MPI_AI_EnableAnr(AiDevId, j);
        if (s32Ret)
        {
            ERRORPRINT("HI_MPI_AI_EnableAnr(%d,%d) failed with %#x\n",AiDevId, j, s32Ret);
            //return HI_FAILURE;
        }
        ...
        {
            s32Ret = HI_MPI_AI_EnableReSmp(AiDevId, j, &s_stAiReSmpAttr);
            if (s32Ret)
            {
                ERRORPRINT("HI_MPI_AI_EnableReSmp ai(%d,%d) fail\n", AiDevId, j);
                //return HI_FAILURE;
            }
...
HI_S32 SAMPLE_StartAenc(HI_S32 s32AencChnCnt)
{
    ...
    {
        ERRORPRINT("invalid aenc payload type:%d\n", stAencAttr.enType);
        return HI_FAILURE;
    }    
    ...
    {            
        ...
        {
            ERRORPRINT("create aenc chn %d err:0x%x\n", AeChn, s32Ret);
            return s32Ret;
        }        
    }
...
HI_S32 SAMPLE_StopAenc(HI_S32 s32AencChnCnt)
{
    ...
    {
        ret = HI_MPI_AENC_DestroyChn(i);
        if (ret != HI_SUCCESS)
        {
            printf("err: HI_MPI_AENC_DestroyChn %d err:0x%x\n", i, ret);
        } 
    }
    return HI_SUCCESS;
}
...
{
    HI_S32 s32ret, i, max = 0, u32ChnCnt;
...
void * SampleAdecProc(void *parg)
{
    ...
    HI_U32 u32Len = pstAdecCtl->u32PtNumPerFrm * sizeof(HI_U16);
...
static void GetAudioLocalParam( PARAM_CONFIG_AUDIO_ENCODE aParam, AUDIO_BIT_WIDTH_E  *enBitwidth,
                            	AUDIO_SAMPLE_RATE_E *enSamplerate, AUDIO_SOUND_MODE_E  *enSoundmode,
                            	PAYLOAD_TYPE_E  *playloadType )
{
	if( 0 == aParam.bitWidth ) // 1. 位宽
    {
	...
	if( 0 == aParam.encodeType ) // 4. 编码类型
    {
        *playloadType = PT_ADPCMA;
    }
	else if( 1 == aParam.encodeType )
    {
        *playloadType = PT_G711A;
    }
	else
    {
        *playloadType = PT_G726;
    }
}
...
HI_S32 SAMPLE_AiAenc()
{
    ...
    AENC_CHN AeChn;
    SAMPLE_AIAENC_S *pstAiAenc = NULL;
    PARAM_CONFIG_AUDIO_ENCODE aParam;
    int ret;
    
	ret = ParamGetAudio( &aParam );
    if( 0 == ret )
    {
    	GetAudioLocalParam( aParam, &stAioAttr.enBitwidth, &stAioAttr.enSamplerate,
                            &stAioAttr.enSoundmode, &s_enPayloadType );
    	stAioAttr.enWorkmode     = AIO_MODE_I2S_SLAVE;
        stAioAttr.u32ChnCnt      = (0==AiDev) ? 8 : 16;
        stAioAttr.u32EXFlag      = 1;
        stAioAttr.u32FrmNum      = 30;
    ...
    if (HI_TRUE == s_bAioReSample)
    {
        ...
        /* ai 32k -> 8k */
        s_stAiReSmpAttr.u32InPointNum  = 320;
        s_stAiReSmpAttr.enInSampleRate = AUDIO_SAMPLE_RATE_32000;
        s_stAiReSmpAttr.enReSampleType = AUDIO_RESAMPLE_4X1;
    }

    s_s32PointNum = stAioAttr.u32PtNumPerFrm;/* Set s_s32PointNum, AdecAo may use it */
    /* config audio codec */
    //SAMPLE_TW2865_CfgAudio(stAioAttr.enSamplerate);
    
    s32AiChnCnt = stAioAttr.u32ChnCnt; 
    s32AencChnCnt = MAX_AENC_CHN_NUM;

    /* enable AI channle */
    s32Ret = SAMPLE_StartAi(AiDev, s32AiChnCnt, &stAioAttr);
    if (s32Ret != HI_SUCCESS)
    {
        ERRORPRINT("SAMPLE_StartAi failed\n");
        return HI_FAILURE;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        ERRORPRINT("SAMPLE_StartAenc failed\n");
        return HI_FAILURE;
    }
    else
    {
        //CORRECTPRINT("SAMPLE_StartAenc successed\n");
    }
    ...
    for (i=0; i<s32AencChnCnt; i++)
    {
        ...
        if (s32Ret != HI_SUCCESS)
        {
            ERRORPRINT("Ai(%d,%d) bind to Aenc(%d) err \n", AiDev, AiChn, AeChn);
            return s32Ret;
        }
    } 
...
HI_S32 SAMPLE_AdecAo()
{
    ...
    if (s_s32PointNum > 0)  /* if AiAenc has been run over, here use it's u32PtNumPerFrm */
    {
        stAioAttr.u32PtNumPerFrm = s_s32PointNum;
    }
    ...
    pstAoAdec->u32PtNumPerFrm = stAioAttr.u32PtNumPerFrm;
...
HI_S32 SAMPLE_AiAo()
{
    ...
    /* config aio dev attr */    
    stAioAttr.enWorkmode = AIO_MODE_I2S_SLAVE;
    stAioAttr.u32ChnCnt = (0==AiDev) ? 4 : 16;
    stAioAttr.enBitwidth = AUDIO_BIT_WIDTH_16;
