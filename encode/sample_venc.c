...
void SAMPLE_VENC_Usage(char* sPrgNm)
{
    ...
    printf("\t 4) svc-t H264\n");
	printf("\t 5) H264 P refurbish I slice,normal@1080+I macro block@1080+Islice @1080\n");
	printf("\t 6) H265 P refurbish I slice,normal@1080+I macro block@1080+Islice @1080\n");
	printf("\t 7) Gop_Mode_SmartP for H.265@1080P@normalP +H.265@1080P@smartP \n");
	printf("\t 8) Thumbnail of 1*1080p jpeg.\n");
	printf("\t 9) AdvSmartP H.265@1080@30fps+H.265@1080@30fps.\n");
...
HI_S32 SAMPLE_VENC_1080P_CLASSIC(HI_VOID)
{
    ...
    PIC_SIZE_E enSize[3] = {PIC_HD1080, PIC_5M,PIC_QVGA};
	HI_U32 u32Profile = 0;
    ...
    ******************************************/
    ...
    if(PIC_3kx3k == enSize[0])
    {
        s32ChnNum = 2;
        enSize[1] = PIC_HD720;
    }

	if(PIC_UHD4K == enSize[0])
	{
		s32ChnNum = 2;
		enSize[1] = PIC_HD720;
	}
	else if(PIC_HD1080 == enSize[0])
	{
		s32ChnNum = 2;
		enSize[1] = PIC_HD1080;
	}
...
HI_S32 SAMPLE_VENC_1080P_MJPEG_JPEG(HI_VOID)
{
    ...
    ******************************************/
	...
	if (HI_SUCCESS != s32Ret)
    {
	   SAMPLE_PRT("SAMPLE_COMM_VENC_GetGopAttr!\n");
	   goto END_VENC_MJPEG_JPEG_5;
    }
    s32Ret = SAMPLE_COMM_VENC_Start(VencChn, enPayLoad, \
                                    gs_enNorm, enSize, enRcMode, u32Profile,&stGopAttr);
    if (HI_SUCCESS != s32Ret)
    {
        SAMPLE_PRT("Start Venc failed!\n");
        goto END_VENC_MJPEG_JPEG_5;
...
/******************************************************************************
* function :  H264 P refurbish I slice,normal@1080+I macro block@1080+Islice @1080 
******************************************************************************/
HI_S32 SAMPLE_VENC_IntraRefresh_H264(HI_VOID)
{
...
//	printf("s32ChnNum = %d\n",s32ChnNum);
    ...
    ******************************************/
	...
	if(s32ChnNum >= 1)
	{
		...
		stIntraRefresh.bRefreshEnable = HI_FALSE;
...
/******************************************************************************
* function :   H265 P refurbish I slice,normal@1080+I macro block@1080+Islice @1080
******************************************************************************/
HI_S32 SAMPLE_VENC_IntraRefresh_H265(HI_VOID)
{
    PAYLOAD_TYPE_E enPayLoad[3]= {PT_H265, PT_H265,PT_H265};
    PIC_SIZE_E enSize[3] = {PIC_HD1080, PIC_HD1080,PIC_QVGA};
	HI_U32 u32Profile = 0;
...
******************************************************************************/
HI_S32 SAMPLE_VENC_Gop_Mode_SmartP(HI_VOID)
{
	PAYLOAD_TYPE_E enPayLoad[3]= {PT_H265, PT_H265,PT_H264};
	PIC_SIZE_E enSize[3] = {PIC_HD1080, PIC_HD1080,PIC_QVGA};
	VENC_GOP_MODE_E enGopMode[3] = {VENC_GOPMODE_NORMALP,VENC_GOPMODE_SMARTP,VENC_GOPMODE_NORMALP};
	HI_U32 u32Profile = 0;
...
HI_S32 SAMPLE_VENC_1080P_JPEG_Thumb(HI_VOID)
{
    ...
    ******************************************/
    ...
    strncpy((char *)stIspDCF.au8ImageDescription,"Thumbnail test",DCF_DRSCRIPTION_LENGTH);
...
/******************************************************************************
* function AdvSmartP :  H.265@1080@30fps+H.265@1080@30fps

******************************************************************************/
HI_S32 SAMPLE_VENC_AdvSmartP_CLASSIC(HI_VOID)
{
    ...
    VENC_CHN VencChn;
    SAMPLE_RC_E enRcMode= SAMPLE_RC_CBR;
	HI_BOOL bEnableAdvSmartP=HI_TRUE;
...
//	printf("s32ChnNum:%d,Sensor Size:%d\n",s32ChnNum,enSize[0]);
    ...
    ******************************************/
	...
	if(s32ChnNum >= 1)
	{
		...
		bEnableAdvSmartP = HI_TRUE;
		s32Ret = HI_MPI_VENC_EnableAdvSmartP(VencChn,bEnableAdvSmartP);
	    if (HI_SUCCESS != s32Ret)
	    {
...
int test_main(int argc, char* argv[])
{
    ...
    switch (*argv[1])
    {
        ...
        case '4':/* H.264 Svc-t */
            ...
            break;
		case '5':/* H264 P refurbish I slice,normal@1080+I macro block@1080+Islice @1080*/
            s32Ret = SAMPLE_VENC_IntraRefresh_H264();
            break;
		case '6':/* H265 P refurbish I slice,normal@1080+I macro block@1080+Islice @1080*/
    		s32Ret = SAMPLE_VENC_IntraRefresh_H265();
            break;
		case '7':/*Gop_Mode_SmartP for H.265@1080P@normalP +H.265@1080P@smartP  */
			s32Ret = SAMPLE_VENC_Gop_Mode_SmartP();
			break;
        case '8':/* Thumbnail of 1*1080p jpeg  */
            s32Ret = SAMPLE_VENC_1080P_JPEG_Thumb();
            break;
		case '9':/*AdvSmartP H.265@1080@30fps+H.265@1080@30fps*/
            s32Ret = SAMPLE_VENC_AdvSmartP_CLASSIC();
            break;
        default:
